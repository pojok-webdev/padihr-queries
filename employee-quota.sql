...
C.prev_level,
case 
when (timestampdiff(YEAR,joindate,now())=0) then '0' 
when (timestampdiff(YEAR,joindate,now())=1) then 'prorate' 
when (timestampdiff(YEAR,joindate,now())>1) then B.quota 
else 'undefined' end curleavequota  
from(
select a.fullname,b.id_employee,min(b.tgl_mulai)joindate from employee a 
left outer join employment b on b.id_employee=a.id_employee 
group by a.fullname,b.id_employee) A
